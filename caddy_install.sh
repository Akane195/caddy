...
#       Added armv7l arrch64 support by Akane195 
#=================================================
...
Download_caddy(){
	...
	if [[ ${bit} == "i386" ]]; then
		...
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://caddyserver.com/download/linux/arm64${extension_all}" && caddy_bit="caddy_linux_arm64"
	elif [[ ${bit} == "armv7l" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://caddyserver.com/download/linux/arm7${extension_all}" && caddy_bit="caddy+linux_arm7"
	else
...
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Akane195/caddy/master/caddy_centos -O /etc/init.d/caddy; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Akane195/caddy/master/caddy_debian -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
